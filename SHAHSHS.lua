-- Haimiya Script Clean V3 (Super Fast Scan LIMITER)
gg.alert("Haimiya Script Aktif")

-- ==================================================
-- MESIN INJECT DAMAGE (1X SCAN + LIMITER JARAK MEMORI)
-- ==================================================
local function injectDamageOtomatis(stats, n_HP, n_ATK)
    -- RAHASIA KECEPATAN: Tambahan '::120' di belakang string
    -- GG cuma butuh sepersekian detik karena nggak nyari di seluruh memori buta
    local searchString = string.format("%d;%d;%d;%d;%d::120", stats.HP, stats.ATK, stats.DEF, stats.MDEF, stats.SPD)
    ...
    for _, v in ipairs(results) do
        if v.value == stats.HP then
            table.insert(edits, {address = v.address, flags = gg.TYPE_DWORD, value = n_HP})
        elseif v.value == stats.ATK then
            table.insert(edits, {address = v.address, flags = gg.TYPE_DWORD, value = n_ATK})
        end
    end
...
function instantHero()
    ...
    if injectDamageOtomatis(stats, new_HP, new_ATK) then
        gg.toast("CHEAT DAMAGE 60K " .. heroName .. " AKTIF!")
    else
        gg.alert("Gagal menemukan hero " .. heroName .. "!\nPastikan aktifkan di dalam pertandingan.")
    end
end

-- ==================================================
-- FITUR DAMAGE ROLE LAINNYA
-- ==================================================
function damageBooster()
    ...
    if injectDamageOtomatis(stats, n_HP, n_ATK) then gg.toast("CHEAT DAMAGE AKTIF!") else gg.alert("Gagal menemukan hero!") end
end
