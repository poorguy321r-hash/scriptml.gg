...
function skipEnamMenit()
    ...
    if #hasil == 0 then
        gg.toast("Gagal Skip Waktu: Memori waktu belum terbaca (coba tunggu detik game jalan).")
        return
    end
    ...
    if target == nil then
        gg.toast("Error Skip Waktu: Memori waktu tidak sinkron.")
        return
    end
...
-- =========================================================
-- FITUR HERO SPESIAL (HP, ATK, SPD + AUTO SKIP WAKTU)
-- =========================================================
function instantHero()
 ...
 gg.searchNumber(string.format("%d;%d;%d", new_HP, stats.ATK, stats.SPD), gg.TYPE_DWORD)
 gg.refineNumber(stats.ATK, gg.TYPE_DWORD)
 gg.getResults(100)
 gg.editAll(new_ATK, gg.TYPE_DWORD)
 gg.clearResults()

 -- EKSEKUSI 3: Scan dan Ubah MOVEMENT SPEED
 gg.searchNumber(string.format("%d;%d;%d", new_HP, new_ATK, stats.SPD), gg.TYPE_DWORD)
 gg.refineNumber(stats.SPD, gg.TYPE_DWORD)
 gg.getResults(100)
 gg.editAll(new_SPD, gg.TYPE_DWORD)
 gg.clearResults()

 gg.toast("CHEAT HERO SPESIAL AKTIF ! Mengeksekusi Skip Waktu...")
 
 -- Beri sedikit jeda agar game tidak crash karena memproses terlalu cepat
 gg.sleep(500) 
 
 -- EKSEKUSI SKIP WAKTU OTOMATIS
 skipEnamMenit()
end

-- =========================================================
-- DAMAGE BOOSTER STANDAR (HANYA HP & ATK) - MARKSMAN
-- =========================================================
